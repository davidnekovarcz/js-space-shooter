// Firebase configuration and analytics module for Space Shooter
 ...
 */
export async function syncGameStatsWithFirebase() {
  if (!currentUser) {
  ...
  try {
    ...
    if (docSnap.exists()) {
      ...
      const totalPlays = Object.values(localStorageStats).reduce((sum, game) => sum + game.playCount, 0);
      let mostPlayed = null;
 ...
 */
export function trackFirebaseEvent(eventName, parameters = {}) {
  if (analytics) {
